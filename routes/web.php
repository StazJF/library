...
Route::get('users/print', [UserController::class, 'printAll'])->name('users.print');
    ...
    // Book audit module (staff/admin only)
    Route::prefix('audit')->middleware('role:admin,staff')->group(function () {
        Route::get('/', [AuditController::class, 'index'])->name('audit.index');
        Route::get('/create', [AuditController::class, 'create'])->name('audit.create');
        Route::post('/', [AuditController::class, 'store'])->name('audit.store');
